  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-16 px-6 pb-24 pt-10 lg:grid-cols-[1.1fr_0.9fr] lg:px-10 lg:pb-28 lg:pt-16">
      <SectionReveal className="lg:pr-4">
        <div className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-white/90 px-4 py-2 text-sm text-slate-600 shadow-sm">
          <span className="h-2 w-2 rounded-full bg-surge animate-pulseSoft" />
          AI-native SOC coverage for overwhelmed security teams
        </div>

        <h1 className="mt-8 max-w-3xl font-display text-5xl font-bold leading-tight text-ink md:text-6xl">
        ...
        <div className="mt-8 flex flex-col gap-4 sm:flex-row">
          <Link
            to="/pricing"
            className="rounded-full bg-surge px-6 py-4 text-center text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-sky-500"
          >
            See Pricing
          </Link>
          <Link
            to="/platform"
            className="rounded-full border border-slate-200 bg-white px-6 py-4 text-center text-sm font-semibold text-ink transition hover:-translate-y-0.5 hover:border-sky-200 hover:bg-sky-50"
          >
            Explore Platform
          </Link>
        </div>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {metrics.map((metric, index) => (
            <div
              key={metric.label}
              className="rounded-3xl border border-slate-200 bg-white/90 p-5 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-surge/20 hover:bg-white"
